...
class Request
{
    ...
    function setGlobal()
    {
        ...
        $_COOKIE = $this->cookie;
        $_SERVER = $this->server;
        if(isset($this->server['HTTP_CONTENT_TYPE']) && $this->server['HTTP_CONTENT_TYPE'] == 'application/json' && $this->body)
        {

            $bodyParams = @json_decode($this->body, 1);
            if($this->server['REQUEST_METHOD'] == 'POST')
            {
                $_POST = is_array($bodyParams) ? array_merge($_POST, $bodyParams) : $_POST;
                $this->post = $_POST;
            } else {
                $_GET= is_array($bodyParams) ? array_merge($_GET, $bodyParams) : $_GET;
                $this->get = $_GET;
            }
