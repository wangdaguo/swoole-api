...
class Controller extends Object
{
    ...
    function get($key, $default = '', $intval = false)
    {
        return $this->value($_GET, $key, $default, $intval);
    }

    function post($key, $default = '', $intval = false)
    {
        return $this->value($_POST, $key, $default, $intval);
    }
    ...
    {
        $json = array('errcode' => $code, 'errmsg' =>$message, 'data' => $data);
        if (!empty($_REQUEST['jsonp']))
        {
            $this->http->header('Content-type', 'application/x-javascript');
            echo $_REQUEST['jsonp'] . "(" . json_encode($json) . ");";
        }
        else
        {
