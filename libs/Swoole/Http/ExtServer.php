...
class ExtServer implements Swoole\IFace\Http
{
    ...
    function onRequest(\swoole_http_request $req, \swoole_http_response $resp)
    {
        ...
        $php->shmCache->incr(date('i') . $_SERVER['REQUEST_URI'], 1, 180);
        $php->profile->init($_SERVER['REQUEST_URI']);
        try
        {
            try
            {
            ...
            {
                $php->log->error($_SERVER['REQUEST_URI'] . "\t" . json_encode($_REQUEST) . "\t" .$e->getTraceAsString());
                $php->profile->log();
        ...
        {
            $resp->status(500);
            $message = defined('DEBUG') && strtolower(DEBUG) == 'on' ? $e->getMessage() . "<hr />" . nl2br($e->getTraceAsString()) : '';
            $php->log->error($_SERVER['REQUEST_URI'] . "\t" . json_encode($_REQUEST) . "\t" .$e->getTraceAsString());
            $php->profile->log();
            $resp->end($message);
        }
